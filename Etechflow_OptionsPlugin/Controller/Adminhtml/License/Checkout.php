...
class Checkout extends Action
{
    ...
    public function execute(): ResultInterface
    {
        ...
        try {
            ...
            if ($stripeKey === '') {
                return $result->setData(['error' => 'Stripe is not configured. Go to Stores > Configuration > eTechFlow > Extra Options Plugin > Payment (Stripe) and add your Stripe Secret Key.']);
            }
            ...
            $successUrl = (string) $this->getUrl('efopt/license/activated')
                . '?session_id={CHECKOUT_SESSION_ID}'
                . '&plan='   . urlencode($plan)
                . '&domain=' . urlencode($domain)
                . '&name='   . urlencode($name)
                . '&email='  . urlencode($email);
            $cancelUrl  = (string) $this->getUrl('efopt/license/gate');
    ...
    {
        $c = strtolower(trim((string) $this->scopeConfig->getValue('etechflow_options/payment/stripe_currency')));
        return $c !== '' ? $c : 'usd';
    }
    ...
    {
        $catalog = [
            'eo_starter'      => ['amount' => 1900, 'label' => 'Starter Plan',      'mode' => 'payment'],
            'eo_professional' => ['amount' => 4900, 'label' => 'Professional Plan', 'mode' => 'payment'],
            'eo_enterprise'   => ['amount' => 9900, 'label' => 'Enterprise Plan',   'mode' => 'payment'],
        ];
    ...
    private function createCheckoutSession(
        ...
        $payload = [
            ...
            'metadata[plan]'                                  => $plan,
            'metadata[module]'                                => 'options-plugin',
            'metadata[customer_name]'                         => $name,
            'line_items[0][price_data][currency]'             => $currency,
            'line_items[0][price_data][unit_amount]'          => (string) $info['amount'],
            'line_items[0][price_data][product_data][name]'   => 'Extra Options Plugin — ' . $info['label'],
